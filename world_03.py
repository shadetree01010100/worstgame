...
class Player():
    ...
    def move(self, input=0):
        # clamp input to [-1, 1]
        direction = min(max(input, -1), 1)
...
class World():
    ...
    def _draw_stats(self, color=(255, 255, 255)):
        ep_remain = self.font.render('{}'.format(self.EPISODE_LIMIT - self.episodes), 0, color)
        foods_remain = self.font.render('{}'.format(len(self.foods)), 0, color)
        ...
        self.surface.blit(ep_remain, (1, 0))
        self.surface.blit(foods_remain, (1, 16))
